...
$mensaje="Ingrese una clave de empleado!!! ";
$id_user="";
$nu_trabajador="";
$nombre="";
if(isset($_POST['busca'])){
  $id_user=verifica_entrada($_POST['busca']);
 $select ="SELECT id_personal,nombre,a_paterno,a_materno from personal_genius where id_personal like '$id_user'";
$res=$mysql->query($select);
   $datos=$res->fetch_row();
   if($datos){
       $nu_trabajador=$datos[0];
       $nombre=strtoupper($datos[1]." ".$datos[2]." ".$datos[3]); 
   }else
     $mensaje="No existe empleado con la clave ".$id_user;
}
?>
<form name="busqueda" class="form-inline col-md-offset-1" action="#" method="POST" >
  <div class="form-group">
      <label for="busca">Buscar : </label>
      <input type="numero" class="form-control" id="busca" name="busca" placeholder="Clave Empleado">
    </div>
    <button type="submit" class="btn btn-default">Buscar</button>
</form>
<br>
<?php if($nu_trabajador!=""&&$nombre!=""){?>
<div class="row">
...
<h5><strong><center>Se le asignar&aacute;n permisos al empleado:</center></strong>
 <br>No. Empleado: <b><?php echo "G".$nu_trabajador;?></b><br>
...
<div id="debug"></div>
<script src="config/permisos.js"></script>
<?php $mysql->close(); ?>